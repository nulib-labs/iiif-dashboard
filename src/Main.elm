...
import Domain exposing (CheckResult, Project, ProjectResults, Registry, ResultsFile, Status(..), statusLabel, statusRank)
        ...
        , main_ [ class "page-shell" ]
            [ div [ class "intro" ]
                [ div []
                    ...
                    , p [] [ text "Warnings are opinionated, but failures should represent actual deviation from the IIIF spec." ]
                    ]
    ...
    let
        ...
        aggregate =
            if prefix == "image" then
                Dict.get "image.default" checks |> Maybe.map .status |> Maybe.withDefault Unknown

            else
                List.sortBy (.status >> statusRank >> negate) coreResults |> List.head |> Maybe.map .status |> Maybe.withDefault Unknown
            ...
            case defaultResult of
                Just plainResult ->
                    if plainResult.detected |> Maybe.map (String.startsWith version) |> Maybe.withDefault False then
                        ...
                        Dict.get (prefix ++ "." ++ version) checks |> Maybe.map .status |> Maybe.withDefault Unknown
...
miniStatus : String -> Status -> Html msg
miniStatus version status =
